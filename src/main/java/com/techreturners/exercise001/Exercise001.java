...
public class Exercise001 {
    ...
    public double addVat(double originalPrice, double vatRate) {
        ...
        Vat = (double)Math.round(temp * 100d) / 100d;
        return Vat;
    }
    ...
    public String reverse(String sentence) {
        ...
        String ReversedWord = new StringBuffer(sentence).reverse().toString();
        return ReversedWord;
    }
    ...
    public int countLinuxUsers(List<User> users) {
        ...
        List<User> linuxUserList = users.stream().filter(u -> u.getType().equals("Linux")).collect(Collectors.toList());
        return linuxUserList.size();
    }
}
